...
・ ステータス画面：従来型ステータス画面そのままに右側画像ウインドウを追加
 ...
 　※ アクター画像は「拡大顔画像」が標準で描画されますが、  
　　　　ActorPictures.js（「ベーシック＋HDレイアウト」で標準実装）併用で「立ち絵」を、  
　　　　[NLM_CardLayoutMZ.js](https://github.com/nolimits-tukool/NLM_CardLayoutMZ) 併用で「カード画」を、置き換えて描画できます
